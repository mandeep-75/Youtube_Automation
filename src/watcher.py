...
def is_already_processed(video_path):
    ...
    # Check if output directory exists with final videos (ace or tts suffix)
    final_ace = os.path.join(output_dir, "final_video_ace.mp4")
    final_tts = os.path.join(output_dir, "final_video_tts.mp4")

    return os.path.exists(final_ace) or os.path.exists(final_tts)
...
def process_video(video_path):
    ...
    try:
        ...
        subprocess.run(
            [PYTHON_EXE, PIPELINE_SCRIPT, video_path], check=True, cwd=PROJECT_ROOT
        )
